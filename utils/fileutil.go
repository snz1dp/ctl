...
// FileChecksum -
func FileChecksum(filename string, checksum hash.Hash) ([]byte, error) {
	...
	for {
		rc, err := io.ReadFull(reader, buf)
		if rc > 0 {
			if _, err = checksum.Write(buf[:rc]); err != nil {
				return nil, err
			}
		}
		if err != nil {
...
// CreateFileTempWriter 创建临时读取IO
func CreateFileTempWriter(filename string, mode os.FileMode) (string, *os.File, error) {
	tempFile, err := ioutil.TempFile(filepath.Split(filename))
	if err != nil {
		return "", nil, err
	}
...
func (opt *FindFileOption) isInclude(name string) (included bool) {
	if len(opt.Includes) == 0 {
		included = true
		return
	}
	var regex *regexp.Regexp
	for _, incRegex := range opt.Includes {
		regex = opt.includeRegex[incRegex]

		if regex == nil {
			regex = regexp.MustCompile(incRegex)
			opt.includeRegex[incRegex] = regex
		}
		included = regex.FindString(name) != ""
		if included {
			return
		}
	}
...
func (opt *FindFileOption) isExclude(name string) (excluded bool) {
	if len(opt.Excludes) == 0 {
		excluded = false
		return
	}
	var regex *regexp.Regexp
	for _, excRegex := range opt.Includes {
		regex = opt.excludeRegex[excRegex]

		if regex == nil {
			regex = regexp.MustCompile(excRegex)
			opt.excludeRegex[excRegex] = regex
		}
		excluded = regex.FindString(name) != ""
		if excluded {
			return
		}
	}
...
// FindDirFiles -
func FindDirFiles(dir string, opt FindFileOption) (filenames []string, err error) {
	var (
		files []os.FileInfo
		subfs []string
	)
	...
	for _, f := range files {
		if f.IsDir() {
			if !opt.Subdir || opt.isExclude(f.Name()) || !opt.isInclude(f.Name()) {
				continue
			}
			subfs, err = FindDirFiles(path.Join(dir, f.Name()), opt)
			if err != nil {
				return
			}
			filenames = append(filenames, subfs...)
		} else {
			if opt.isExclude(f.Name()) || !opt.isInclude(f.Name()) {
				continue
			}
